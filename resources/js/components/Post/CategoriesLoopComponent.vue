      ...
      <div class="row row-post-loop" id="blog">
              ...
              <div class="card">
                  ...
                  <div class="card-body">
                      ...
                      <!-- disponer enlace hacia archive según nombre de categoría -->
                       <a :href="'/entradas/categoria/' + cats.slug" class="cat-meta" v-for="cats in post.postcategories">
                        {{ cats.name }}
                       </a>
                  </div>
              </div>
          ...
          <div class="d-flex justify-content-center mb-100">
              <Bootstrap5Pagination
                  :data="postsData"   
                  @pagination-change-page="loadPostsCategory"
              />
          </div>
      </div>
    </div>
...
const loadPostsCategory = async (page = 1) => {
  try {
    const response = await axios.get(`/api/postcategory/post/${props.postCategorySlug}?page=${page}`, {
      params: selected,
    });
